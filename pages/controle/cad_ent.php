<?php
    ...
    try {
        ...
        if ($r) {
            if ($_POST['id_controle'] == "") {
                $qtdRes += $r['qtd']; // Adiciona a quantidade existente do estoque
            }
            $id_ctrl  = $r['id'];
        }
    } catch (\Throwable $th) {
    ...
    try {
        ...
        if ($qtd != $qtdRes) {
            // Atualiza o registro na tabela 'medicamento_controle' se a quantidade informada for diferente da quantidade existente
            ...
            // Insere um novo registro na tabela 'medicamento_controle' se a quantidade informada for igual à quantidade existente
